...
const tempRoot = join(process.cwd(), 'tmp-feature-payments');
await rm(tempRoot, { recursive: true, force: true });
await mkdir(tempRoot, { recursive: true });

const config: StackforgeConfig = {
  projectName: 'test-feature-payments',
  packageManager: 'npm',
...
const next = updateConfigForFeature(current, 'feature', 'payments', 'add');
...
if (!afterAdd.features.includes('payments')) throw new Error('feature add failed');

await cleanupFeature(projectRoot, afterAdd, 'feature');
const removed = updateConfigForFeature(afterAdd, 'feature', 'payments', 'remove');
...
if (afterRemove.features.includes('payments')) throw new Error('feature remove failed');

console.log('feature payments test passed');